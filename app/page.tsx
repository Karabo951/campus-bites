  ...
  const [vendors, setVendors] = useState<Vendor[]>([
    {
      ...
      description: 'Burgers, Chips, and Wraps',
      is_open: true,
    },
    {
      id: 'v2',
      name: 'Crunch Cafe',
      description: 'Coffee, Smoothies, and Bakery items',
      is_open: true,
    },
  ]);
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-neutral-800 pb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/orders"
              className="bg-neutral-900 border border-neutral-800 text-xs font-bold px-4 py-2 rounded-xl text-orange-400 hover:text-orange-300 flex items-center gap-2 transition-colors"
            >
              📋 My Orders
            </Link>
            <Link
              href="/enquiries"
              className="bg-neutral-900 border border-neutral-800 text-xs font-bold px-3 py-2 rounded-xl text-amber-400 hover:text-amber-300 transition-colors flex items-center gap-1.5"
            >
              💬 Enquiries
            </Link>
            <Link
                  ...
                  <div className="flex items-center gap-3.5">
                    {v.logo_url ? (
                      <img
                        src={v.logo_url}
                        alt={`${v.name} Logo`}
                        className="w-12 h-12 rounded-2xl object-cover border border-neutral-800 shrink-0"
                      />
                    ) : (
                      <div className="w-12 h-12 rounded-2xl bg-neutral-950 border border-neutral-800 flex items-center justify-center text-xl shrink-0">
                        🏪
                      </div>
                    )}
                    <div>
                      <h3 className="text-xl font-bold text-white">{v.name}</h3>
                      <p className="text-xs text-neutral-400 mt-0.5 line-clamp-2">{v.description}</p>
                    </div>
                  </div>
